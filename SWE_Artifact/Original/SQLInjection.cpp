...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
    ...
    records.clear();
    std::regex injections(R"((\bor\b|\band\b).*=.*|--|;|union\s+select)", std::regex::icase); // regex used to hold possible injection substrings within the SQL query

    if(std::regex_search(sql, injections))  // regex_search looks through sql statement for possible SQL injection substrings using injections, if true that means there is a SQL injection detected, false its a clean query
    {
        std::cout << "SQL Injection detected. Query rejected." << std::endl; // output injection error statement
        return false; 
	}
